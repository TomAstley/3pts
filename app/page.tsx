...
export default function Home() {
  ...
  useEffect(() => {
    ...
    tl.to(
      s1.current,
      { y: -100, rotate: 6, duration: 4, ease: "sine.inOut" },
      0,
    );

    const gaps = [0.5, 0.4, 0.3, 0.25, 0.2, 0.15];
    let t = 0.25;
    ...
    tl.to(s1.current, { scale: 0.2, duration: 0.12, ease: "sine.out" }, 5).to(
      s1.current,
      { scale: 0.15, x: -180, duration: 0.18, ease: "sine.in" },
      5.12,
    );
    ...
    tl2.to(
      lThreePts.current,
      { opacity: 0, duration: 0.4, ease: "quad.in" },
      4,
    );
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full overflow-hidden">
      <Image
        ...
        src="/sphere.svg"
        alt="Sphere"
        className="w-full h-full scale-400 absolute blur-[2px]"
        fill
      />
      <Image
        ref={s2}
